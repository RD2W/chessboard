...
$(BIN_DIR)/$(APP_NAME):
	...
	go build $(OPTIMIZATION) -o $(BIN_DIR)/$(APP_NAME) ./cmd/main.go
...
build-debug:
	...
	go build -race -o $(BIN_DIR)/$(APP_NAME)-debug ./cmd/main.go
...
build-linux:
	...
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build $(OPTIMIZATION) -o $(DIST_DIR)/$(APP_NAME)-linux-amd64 ./cmd/main.go
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build $(OPTIMIZATION) -o $(DIST_DIR)/$(APP_NAME)-linux-arm64 ./cmd/main.go
	@echo "Linux binaries: $(DIST_DIR)/$(APP_NAME)-linux-*"

build-windows:
	@mkdir -p $(DIST_DIR)
	@echo "Building Windows binaries..."
	GOOS=windows GOARCH=amd64 go build $(OPTIMIZATION) -o $(DIST_DIR)/$(APP_NAME)-windows-amd64.exe ./cmd/main.go
	@echo "Windows binary: $(DIST_DIR)/$(APP_NAME)-windows-amd64.exe"

build-macos:
	@mkdir -p $(DIST_DIR)
	@echo "Building macOS binaries..."
	GOOS=darwin GOARCH=amd64 go build $(OPTIMIZATION) -o $(DIST_DIR)/$(APP_NAME)-darwin-amd64 ./cmd/main.go
	GOOS=darwin GOARCH=arm64 go build $(OPTIMIZATION) -o $(DIST_DIR)/$(APP_NAME)-darwin-arm64 ./cmd/main.go
...
install:
	...
	go install -ldflags="$(LD_FLAGS)" ./cmd/main.go
